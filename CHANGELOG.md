...
- 桌面端和移动端加入“支持项目”入口，集中展示模型 API 邀请链接、返利披露与微信支持二维码；GitHub 首页同步展示这些信息。
...
- 全量验证通过：Server 586 项、Web 201 项、Cloudflare 74 项、EdgeOne 8 项、Hosted URL 8 项、Relay 5 项，并完成源码干净安装与线上多账号隔离冒烟测试。
- 独立代码审查最终结果为 `PASS`；线上桌面版与强制手机版均完成真实浏览器 DOM、布局和控制台检查。
