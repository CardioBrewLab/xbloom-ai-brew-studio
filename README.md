...
- **Hosted 云端版**：浏览器直接使用，支持多用户、账号隔离、个人模型配置、每个用户自己的 xBloom 云端同步与小红书辅助调研。
...
| 小红书         | 直接在网站扫码；Cookie 按当前浏览器/账号加密隔离                                                       | 可选本地助手                                         |
| 手机/电脑界面  | 自动识别设备，也可手动选择手机版或电脑版                                                               | 同一前端，规则一致                                   |
...
7. 如果调研需要小红书，点击顶栏的小红书状态。手机端会通过小红书官方入口唤起 App 确认，电脑端展示二维码；确认后返回网页，登录状态会自动同步。未登录时生成和 xBloom 上传照常使用。
8. 页面默认自动识别手机或电脑；顶栏的 **界面** 选择器可以固定为手机版或电脑版。
...
- xBloom 凭据只用于当前账号的 xBloom 云端操作。Hosted 小红书 Cookie 使用同一应用数据密钥加密，并按匿名浏览器或注册账号隔离；本地版仍使用本机助手存储。
