...
本文说明 0.2.2 Windows 安装包和多用户 Hosted 云端版的发布流程。它是发布检查清单，不是线上部署结果记录；最终发布说明请使用本次命令的真实输出。
...
| 百分号路径                        | 在包含 `%TEMP%` 等字样的合法 Windows 路径中，服务仍能启动并写入日志                   |
| 第一次运行 `install-windows.bat`  | 项目内 Node.js 运行时、依赖、工作区构建、空白本地 `.env` 和启动准备完成               |
| 可选组件失败                      | XHS helper 或桌面快捷方式失败只产生警告，核心应用仍完成安装                           |
| NTFS/ReFS 权限                    | `.env`、`data/`、助手 runtime 和 Edge proxy secret 只对当前用户及系统管理身份开放     |
| exFAT/FAT 权限                    | 安装器显示醒目警告，测试记录卷不具备按用户 ACL 的事实                                 |
...
4. 创建与根 `package.json` 完全一致的 tag（当前为 `v0.2.2`），并使用仓库已有的发布 workflow 发布 GitHub Release。workflow 会拒绝版本不匹配的 tag，并从 tag 对应版本打包被 Git 跟踪的文件；打 tag 前确认所有公开文档都已被 Git 跟踪。
